...
class ClientHandler implements Runnable {
    ...
    public void run() {
        try (BufferedReader in = new BufferedReader(new InputStreamReader(clientSocket.getInputStream()));
            ...
            if (isSportRoute(fileRequested)) {
                handleSportRoute(method, fileRequested, out);
            } else if (!isAppRoute(fileRequested)) {
    ...
    private void handleSportRoute(String method, String fileRequested, PrintWriter out) throws IOException {
        Service serve = LambdaServer.findHandler(method, fileRequested);
        if (serve != null) {
            String response = serve.getValue(fileRequested);
            List<JSONObject> countries = Countries.getCache(fileRequested, response);
            if (!countries.isEmpty()) {
                sendHttpResponse(out, "200 OK", "application/json", countries.toString());
            }
        }
    }
